...
Considered are patients with abdominal tumors in proximity to either the bowel, stomach, or duodenum and prostate cancer in proximity to the rectum. For prostate cases urethra and bladder are ignored. These patients received 5-fraction SBRT treatments at the MR-Linac system (MRIdian, Viewray). All patients were planned and treated according to institutional practice. In addition to the simulation MR and CT scans, daily MR scans were performed for online adaptive radiotherapy. Tumors and OARs in a 2 cm ring around the tumor were recontoured according to institutional guidelines and daily adaptive treatment plans were created. In each fraction the dose distributions were reoptimised, to adapt to inter-fractional changes, without altering the prescription dose.

\subsection{Motivation}
Goal was to find out for which target reducing number of fractions is attractive or suitable. Search for a tumor site that is a prospective candidate to apply the reduce number of fractions. Whether it is possible to reduce number of fraction we look at correlation between distance and sparing factors and look at order of sparing factor.


\subsection{Methods}
Patient data was collected in a manual manner to get an overview of the distance between OAR and tumor volumes in relation to sparing factor. In total $30$ patients were extracted manually from the treatment planning system, $10$ for each target. In case of the prostate target for $5$ patients the target was the prostate CTV and $5$ patients for boost target volume. For each patient extracted were:\\

\textbf{Target:} $10$ patients were extracted for each tumor called target that was either pancreas, adrenal glands or prostate.

\textbf{OAR:} For each patient at least one of the OAR was extracted. There were two criteria which qualified OAR to be registered. The first was if the OAR was dose-limiting and the second if the OAR was in close vicinity: Every OAR in one patient that was as close to the tumor as the dose-limiting OAR in at least one fraction, was tracked as well. Only patients were collected, where the dose-limiting OAR did not change during the treatment.\\
...
\textbf{Distance:} Distance between target and OAR was the shortest measured distance of the sagittal, transversal or coronal plane. In principal it there was no overlap allowed between GTV and OAR. However if there was an overlap the furthest radial distance of the overlap was measured.\\

\textbf{Dose to Normal Tissue:} $d^\text{N}$ was measured in the dose exceeded in percentage of volume. This information could directly be extracted from the DVH calculated in the treatment planning system.\\

\textbf{Dose to Target:} $d^\text{T}$ was measured in the dose exceeded in $95\%$ of the target volume.\\

A table of which indicators for which target was chosen can be seen in Tab.~\ref{tab:indicators_data}. The predictors were extracted for each tumor-OAR pair for $6$ treatment plans in each patient, corresponding to the the $5$ delivered plans and the initial plan based on the planning MR.
...
\begin{figure}[!htb]
    ...
    \caption{10 pancreas patients treated at the MR-Linac. Sparing factors dependent on distance $w$ grouped by OAR. Multiple OAR were collected per patient and not only the dose-limiting OAR.}
...
\begin{figure}[!htb]
    ...
    \caption{10 adrenal gland patients treated at the MR-Linac. Sparing factors dependent on distance $w$ grouped by OAR. Multiple OAR were collected per patient and not only the dose-limiting OAR.}
    \label{fig:0}
\end{figure}
...
\begin{figure}[!htb]
    ...
    \caption{10 prostate patients treated at the MR-Linac. Sparing factors dependent on distance $w$ grouped by target volume BST (prostate boost volume) and PRO (prostate CTV).}
