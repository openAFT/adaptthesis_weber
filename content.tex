...
\begin{equation}\label{eq:value_function}
    ...
    r_t(d, B^{\text{T}}_{t-1}) + \ldots \Biggr.\\
    & \Biggr. \sum_{\delta'}P(\delta')v_{t+1}(\delta', B^{\text{T}}_{t-1} + d + \frac{d^2}{(\alpha / \beta)_\text{T}}) \Biggr]
    \end{split}
\end{equation}
...
In each fraction $t$, the maximum likelihood estimator of the mean of the sparing factor distribution given by
...
with shape $k$ and scale $\theta$ the hyperparameters. The maximum a posterior estimator for the standard deviation in fraction $t$ is then given by
...
\begin{figure}
    ...
    \caption{Adrenal gland tumor target with three OAR shown are sparing factors dependent on distance between OAR and tumor.}
    \label{fig:3}
\end{figure}
