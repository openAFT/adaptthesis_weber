...
\begin{table}[!htb]
    \centering
    \caption{Comparison of accumulated BED$_3$ for Uniform Fractionation (UF) and Adaptive Fractionation (AF). The number of fraction used to complete the tumor prescription dose of $72$ Gy is given by $n_{\text{frac}}$.}
    \begin{tabular}{cl|cc}
    \toprule
...
\paragraph{Patient $3$:} Applying the treatment model to the real sparing factor data of patient $3$, the cumulative BED$_3$ for $C=0$ results in $76.4$ Gy. Comparing this to Uniform Fractionation, delivering $8$ Gy physical dose to the tumor in each fraction yields $91.8$ Gy cumulative BED BED$_3$. Thus, Adaptive Fractionation for this specific patient and treatment model surpasses OAR sparing by delivering $15.4$ Gy less cumulative BED$_3$ compared to Uniform Fractionation. Applying Adaptive Fractionation with the optimal parameter $C_4=2.8$ results in the treatment terminating with $2$ fractions, while delivering less cumulative BED BED$_3$ than Adaptive Fractionation $C=0$. On average, it is expected that the treatment is finished in $4$ fractions, but for patient $3$ the treatment is terminated after two fractions while accumulating $1.4$ BED less compared to Adpative Fractionation $C=0$. In Tab.~\ref{tab:result_treat_patient} the comparison is summarised.

\paragraph{Patient $7$:} Applying the treatment model to the real sparing factor data of patient $7$, the cumulative BED$_3$ for $C=0$ results in $68.4$ Gy. Comparing this to Uniform Fractionation, delivering $8$ Gy physical dose to the tumor in each fraction yields $79.0$ Gy cumulative BED$_3$. Thus, Adaptive Fractionation for this specific patient and treatment model surpasses OAR sparing by delivering $10.6$ Gy less cumulative BED$_3$ compared to Uniform Fractionation. Applying Adaptive Fractionation with the optimal parameter $C_4=2.0$ results in the same treatment, terminating with $5$ fractions, while delivering the same cumulative BED$_3$ as Adaptive Fractionation $C=0$.

\paragraph{Patient $13$:} For patient $13$ the cumulative BED$_3$ for Adaptive Fractionation $C=0$ results in $120.3$ Gy. Comparing this to Uniform Fractionation, delivering $8$ Gy physical dose to the tumor in each fraction yields $119.6$ Gy cumulative BED$_3$. Thus, Adaptive Fractionation delivers $0.7$ Gy less cumulative BED$_3$ compared to Uniform Fractionation. Applying Adaptive Fractionation with the optimal parameter $C_4=2.0$ gives the same results, while using all $5$ fractions.
...
Applying the treatment model to the synthetic patient, the cumulative BED$_3$ for $C=0$ results in $80.8$ Gy. Comparing this to uniform fractionation (delivering $8$ Gy physical dose to the tumor in each fraction) yields an immediate OAR BED of $18$ Gy for each fraction with $\delta=0.75$ and an immediate OAR BED of $14.21$ Gy for $\delta_2=0.65$, resulting in $86.2$ Gy (rounded) cumulative BED$_3$. Thus, Adaptive Fractionation for this specific patient and treatment model surpasses OAR sparing by delivering $5.4$ Gy less cumulative BED$_3$ compared to uniform fractionation. Applying Adaptive Fractionation with $C=1.2$ and $C=3.0$ results in the treatment terminating with $4$ respectively $2$ fractions (see Fig.~\ref{fig:treat_patient_synthetic}) while still delivering less cumulative BED$_3$ than uniform fractionation and reaching prescribed tumor dose of $72$ Gy. In Tab.~\ref{tab:result_treat_fraction_4} the comparison is summarised.

\begin{table}[!htb]
    \centering
    \caption{Comparison of accumulated BED$_3$ for Uniform Fractionation (UF) and Adaptive Fractionation (AF). The number of fraction used to complete the tumor prescription dose of $72$ Gy is given by $n_{\text{frac}}$. Sparing factors are $\delta_t=0.75$ except for $\delta_2=0.65$}
    \begin{tabular}{cl|cc}
    \toprule
        Patient & Fractionation & $B_5^{\text{N}}$ [Gy] & $n_{\text{frac}}$ \\
    \midrule
...
The Adaptive Fractionation paradigm exploiting day-to-day variations in the distance of the tumor from the dose-limiting OAR was successfully extended to reducing number of fractions. Based on this study considering 5-fraction SBRT treatments of two pancreas tumor patients in proximity to bowel and one adrenal glands tumor in proximity to stomach, it is concluded that the extension brings a benefit to one pancreas patients in terms of OAR dose sparing and reducing number of fractions. However, no other patients show a similar amount of interfraction motion and OAR-to-tumor proximity to substantially benefit from Adaptive Fraction with reduced number of fractions. For one pancreas and one adrenal glands patient Adaptive Fractionation did not bring a benefit to the patients in terms of reducing number of fractions. There is very small amount of inter-fractional motion for prostate patients in proximity to rectum for either CTV or integrated boost volume for Adaptive Fractionation to be beneficial.
...
If a synthetic patient shows no deviation at all with sparing factors appearing at $\mu = 0.75$. A uniformly fractionated treatment plan yields the limiting $90$ Gy cumulative BED$_3$ for a tumor prescription dose of $72$ Gy. Assuming an Adaptive Fractionation treatment model with a low variation in sparing factors, such that practically there is no variation expected at all and a constant $C=0$ (see Fig.~\ref{fig:treat_fraction_0}), the policy conforms to uniform fractionation. Increasing $C$ yields uniform fractionation policy using fewer fractions and resulting in higher $B_5^{\text{N}}$.
\begin{figure}[!htb]
    \centering
...
Increasing the expected variation in sparing factors to a standard deviation of $\sigma=0.05$ seen in Fig.~\ref{fig:treat_fraction_1} for otherwise equal treatment model, yields higher cumulative BED$_3$ compared to a $\sigma=0.001$. For $C=0$ less dose is applied in the first three fractions compared to the treatment model with $\sigma=0.001$, as a variation in favour of lower sparing factors is expected. As no lower sparing factors appear the dose is steadily increased. Looking at the treatment model with $C=4.4$ the treatment is finished after 4 fractions with a cumulative BED$_3$ $3.5$ Gy higher compared to uniform fractionation.
...
For a patient model with zero variation in sparing factor and treatment model with large expected variation the optimal policy yields a suboptimal dose delivery reflected by the higher cumulative BED$_3$ compared to a treatment model that assumes low variation.
...
\begin{table}[!htb]
    \centering
    \caption{Comparison of accumulated BED$_3$ for Uniform Fractionation (UF) and Adaptive Fractionation (AF). The number of fraction used to complete the tumor prescription dose of $72$ Gy is given by $n_{\text{frac}}$. Sparing factors are $\delta_t=0.75$ except for $\delta_2=0.85$}
    \begin{tabular}{l|cc}
    \toprule
