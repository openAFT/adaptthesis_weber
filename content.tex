...
Described and illustrated are the treatment model regarding policy function, value function and expected remaining number of fractions.
...
The value function $v_t(\delta, B^{\text{T}})$ does not reflect optimal policy for the treatment but rather shows the best possible value the algorithm can secure in each state. The lower the value the less desirable a state. Shown here are the corresponding value functions to the policy functions.
...
\textbf{Overlap Status:} The overlap was chosen as predictor whether the distance $w$ will be small between target and OAR. In the treatment planning system it was checked if in one of the sagittal, transversal or coronal plane there was an overlap between the target volume and OAR.\\

\textbf{Distance:} Distance between target and OAR was the shortest measured distance in all the sagittal, transversal or coronal plane. In principle no overlap was allowed between GTV and OAR. However, if there was an overlap the furthest radial distance of the overlap was measured.\\
...
\begin{table}[!htb]
    \centering
    \caption{Operational quantities collected at the MR-Linac. Explained are target dose definition and volume. Targets are PAN (pancreas), ADN (adrenal glands), prostate (PRO) and BST (prostate boost volume). Overlap specifies the volume upon which the binary status was checked to OAR.}
...
\begin{table}[!htb]
    \centering
    \caption{Variability and average of sparing factors in the pancreas data. Average is measured in mean $\mu$ and variability in standard deviation $\sigma$ for each dose-limiting OAR.}
    \begin{tabular}{l|cc}
    \toprule
        OAR & $\mu$ & mean of $\sigma$ \\
    \midrule
       Bowel    &  $0.854$ & $0.053$ \\
       Duodenum &  $0.821$ & $0.031$ \\
       Stomach  &  $0.884$ & $0.050$ \\
...
\begin{table}[!htb]
    \centering
    \caption{Variability and average of sparing factors in the adrenal glands data. Average is measured in mean $\mu$ and variability in standard deviation $\sigma$ for each dose-limiting OAR.}
    \begin{tabular}{l|cc}
    \toprule
        OAR & $\mu$ & mean of $\sigma$ \\
    \midrule
       Bowel    &  $0.711$ & $0.032$ \\
       Duodenum &  $0.582$ & $0.043$ \\
       Stomach  &  $0.692$ & $0.032$ \\
     
    \bottomrule
    \end{tabular}
    \label{tab:adrenal_glands_sparing_factors}
\end{table}
...
\begin{table}[!htb]
    \centering
    \caption{Variability and average of sparing factors in the prostate data. Average is measured in mean $\mu$ and variability in standard deviation $\sigma$ for each target.}
    \begin{tabular}{l|cc}
    \toprule
        Target & $\mu$ & mean of $\sigma$ \\
    \midrule
       PRO    &  $0.932$ & $0.015$ \\
       BST    &  $1.024$ & $0.014$ \\
    ...
    \bottomrule
    \end{tabular}
    \label{tab:prostate_sparing_factors}
\end{table}
